...
Client.prototype._ircEvents = {
	...
	RPL_WHOISCHANNELS: function(message) {
		...
		this._addData('user', target, 'channels', message.params[2].trim().split(/ +/));
		this._addData('user', target, 'raw', [message.raw]);
	},
...
Client.prototype.privmsg = function(target, message, splitMultipleLines) {
	function col(str){
		var finalstr = str;
		var colourcodes = str.match(/\\*\^([0-9]{1}[0-5]?,[0-9]{1}[0-5]?|((20|1([0-9]?)|[0-9])|[0-9]))/g); // if message contains a colour trigger
		for (var i in colourcodes){ // for each colour trigger
			if (colourcodes[i][0] == "\\") { 
				finalstr = finalstr.replace(colourcodes[i], colourcodes[i].slice(1));
				break;
			}
			var num = colourcodes[i].slice(1).split(","); // divide it into foreground and background
			if (num.length == 1){ // if this trigger is not a background colour
				if (num[0] > 15){ // if this trigger is not a colour
					switch(num[0]){
						case "16": // bold
	...
	var splitmsg = message.match(/.{1,457}/g); // split message into 457 character parts
	if (splitMultipleLines && splitmsg.length > 1){ // if message has more than 1 part and split flag is true
		for (var i in splitmsg){
			this.raw(['PRIVMSG', target, col(splitmsg[i])]); // print each message part one after the other
		}
	} else if(!splitMultipleLines && splitmsg.length > 1) { // if message has more than 1 part and split flag is false
		this.raw(['PRIVMSG', target, col(splitmsg[0])+" ..."]); // trim message end with '...'
	} else {
		this.raw(['PRIVMSG', target, col(splitmsg[0])]); // if no flag is set or if message is only 1 part, print it normally
	}
};

Client.prototype.me = function(target, message) {
	this.raw(['PRIVMSG', target, String.fromCharCode(1) + 'ACTION ' + message + String.fromCharCode(1)]);
};

Client.prototype.ctcp = function(target, type, text) {
	this.raw(['NOTICE', target, String.fromCharCode(1) + type.toUpperCase() + ' ' + text + String.fromCharCode(1)]);
}
