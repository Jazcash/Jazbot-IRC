...
Client.prototype.privmsg = function(target, message, splitMultipleLines) {
	var splitmsg = message.match(/.{1,457}/g);
	if (splitMultipleLines && splitmsg.length > 1){
		for (var i in splitmsg){
			this.raw(['PRIVMSG', target, splitmsg[i]]);
		}
	} else if(!splitMultipleLines && splitmsg.length > 1) {
		this.raw(['PRIVMSG', target, splitmsg[0]+" ..."]);
	} else {
		this.raw(['PRIVMSG', target, splitmsg[0]]);
	}
};
