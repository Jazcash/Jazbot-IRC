...
module.exports = {
			...
			api.hookEvent('*', 'whois', function(whoisdata) {
				if (whoisdata != false){ // if whois was successful
					...
					if (!validator.isIn(username, config.ignore)){ // if user isn't already being ignored
						...
						client.irc.privmsg(target, style.lightgreen+"Ignoring "+nickname);
						JSONFile.writeFileSync(config, "config.json"); // save config
					} else {
						client.irc.privmsg(target, style.lightred+nickname+" is already being ignored");
					}
				} else {
					client.irc.privmsg(target, style.lightred+"User with nickname "+nickname+" could not be found");	
				}
			...
			api.hookEvent('*', 'whois', function(whoisdata) {
				if (whoisdata != false){ // if whois was successful
					...
					if (validator.isIn(username, config.ignore)){ // if user is being ignored
						...
						client.irc.privmsg(target, style.lightgreen+nickname+" is not longer being ignored");
						JSONFile.writeFileSync(config, "config.json"); // save config
					} else {
						client.irc.privmsg(target, style.lightred+nickname+" is already not being ignored");
					}
				} else {
					client.irc.privmsg(target, style.lightred+"User with nickname "+nickname+" could not be found");	
				}
	...
	"!ignorelist":
	{
		...
		"synopsis":"!ignorelist",
		"func":function(api, client, config, sessiondata, style, target, msg, args){
			client.irc.privmsg(target, style.darkblue+"Currently ignored users: "+style.blue+config.ignore.join(", "));
		}
	},
	"!clearignores":
	{
		...
		"func":function(api, client, config, sessiondata, style, target, msg, args){
			client.irc.privmsg(target, style.lightgreen+"All ignored users cleared");
		}
	}
};