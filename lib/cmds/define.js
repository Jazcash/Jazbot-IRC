...
module.exports = {
	"!define": {
      ...
      if (webdef !== undefined){
			...
			if (defs.length <= 0) throw {"message":"^4There are no definitions for \'"+searchTerm+"\'"};
			if (resultNum <= 0) throw {"message":"^4Result number must be greater than 0"};
			if (resultNum > defs.length) throw {"message":"^4There are not that many results"};
