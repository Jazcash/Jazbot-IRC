...
module.exports = {
	"!define": {
      ...
      if (webdef !== undefined){
				defs.push({"def":webdef.children[0].data, "url":webdef.next.attribs.href, "word":$('.vk_ans').text()});
      }
			
			var def = defs[resultNum-1];
			
			if (defs.length <= 0) throw {"message":style.lightred+"There are no definitions for \'"+searchTerm+"\'"};
			if (resultNum <= 0) throw {"message":style.lightred+"Result number must be greater than 0"};
			if (resultNum > defs.length) throw {"message":style.lightred+"There are not that many results"};
			
			var resultNumsStr = (resultNum > 1) ? "" : style.purple+"("+(defs.length)+" results) ";
			var resultNumStr = style.turquoise+resultNum+". ";
			var defStr = style.darkblue+def.word+": "+style.blue+def.def;
			var exampleStr = ("example" in def) ? style.lightgrey+def.example : "";
