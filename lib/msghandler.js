		...
		if (cmdtrigger in this.cmds){ // if cmd exists
			...
			if (args.length > 0){ // specific help	
				...
				if ("!"+cmdname in this.cmds){ // show help
					this.client.irc.privmsg(target, this.style.darkgreen+this.cmds["!"+cmdname].synopsis+" "+this.style.turquoise+this.cmds["!"+cmdname].detail);
				} else { // cmd doesn't exist
					this.client.irc.privmsg(target, this.style.lightred+"The command \'"+cmdname+"\' doesn't exist");
				}
			} else { // list all cmds	
