...
MsgHandler.prototype.handle = function(msg, target){
	...
	/*if (cmdtrigger == "!help"){ // if user issued !help
		var cmdhelp = "!"+args._[0]; // parameter passed to !help
	...
	if (!(cmdtrigger in this.cmds)) return; // if command doesn't exist then end
	var cmd = this.cmds[cmdtrigger]; // the full cmd itself
	
	try {
		args = cmd.parser.parseArgs(args);
	}	catch(err){
		console.log(err);
	}
	...
	//if (args._.length < cmd.requiredargs) return "^4That command requires "+cmd.requiredargs+" arguments. ("+args._.length+" provided)"; // wrong args
	try { // all requirements are validated, attempt to execute command
		...
		if (errormsg !== undefined) {
			this.client.irc.privmsg(target, errormsg);
		}
	} catch (err){
		if (err != "Incorrect arguments"){ // invalid args passed to command
			this.client.irc.privmsg(target, "^4There was an uncaught error in the command '"+cmdtrigger+"', please see the console");
			console.log(err.stack); // log uncaught errors
		} else {
			this.client.irc.privmsg(target, "^4Incorrect arguments for that command ("+cmdtrigger+")");
		}
	}
}
